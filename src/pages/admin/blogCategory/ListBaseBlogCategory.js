...
const ListBaseBlogCategory = () => {
  ...
  const handleDeleteBlogCate = (bcid) => {
    Swal.fire({
      title: "Xóa danh mục",
      ...
      if (rs.isConfirmed) {
        const response = apiDeleteBlogCategory(bcid);
        if (response.success) toast.success(response.message);
        else toast.error(response.message);
        setdata(data.filter((el) => el._id !== bcid));
      }
    });
  };
    ...
    },
    {
      ...
      render: (item, record, index) => (
        <Space size="middle">
          ...
          <span
            onClick={() => {
              handleDeleteBlogCate(record._id);
            }}
            className="cursor-pointer "
          >
...
export default ListBaseBlogCategory;
